...
<head>
    ...
    <script type="text/javascript">
		...
		var gen = document.getElementById("generation");
        	xmlhttp.open("GET", "put_values.php?var=gen2_generation&value="+gen.value+"&t="+Math.random(), true);
        	xmlhttp.send();
                });
		$("#up1").jqxButton({});
		...
		var current_gen = parseFloat(document.getElementById("generation_p").innerHTML)+10;
        	xmlhttp.open("GET", "put_values.php?var=gen2_generation&value="+current_gen+"&t="+Math.random(), true);
        	xmlhttp.send();
                });
		$("#down1").jqxButton({});
	...
	function update(){
        ...
        xmlhttp.onreadystatechange = function() {
            if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
                ...
                document.getElementById("generation_p").innerHTML = data.gen2_generation;
                document.getElementById("load").innerHTML = data.relay2_load;
                var relay_breaker = parseInt(data.relay2_breaker);
                var gen_breaker = parseInt(data.gen2_breaker);
		if(gen_breaker == 1){
                	$("#breaker").jqxButton({ template: 'success'});
                	$("#breaker")[0].value = 'Closed';
		}else{
		...
		if(relay_breaker == 1){
                ...
                document.getElementById("flow").innerHTML = data.relay2_flow;
